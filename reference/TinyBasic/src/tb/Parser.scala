...
object Parser extends RegexParsers:
  ...
  sealed trait Value
  def value: Parser[Value] = logical_op
  ...
  case class LogicalOp(v1: Value, op: Char, v2: Value) extends BinOp(v1, op, v2)
  def logical_op: Parser[Value] =
    opsP(relational_op, Seq('|', '&'), LogicalOp.apply)
