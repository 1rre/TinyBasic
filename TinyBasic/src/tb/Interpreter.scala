...
class Interpreter private (sts: Seq[Statement], debug: Boolean):
  ...
  def runFromEnd(i: Int, cs: Int = 0): Boolean =
    sts(i).command match {
      case IfCommand(_) | ForCommand(_,_) | UntilCommand(_) | WhileCommand(_) => runFromEnd(i+1, cs+1)
      case EndCommand if cs > 0 => runFromEnd(i+1, cs-1)
      case EndCommand => runLn(i+1)
      case _ => runFromEnd(i+1, cs)
    }
    ...
    if (i >= sts.size)
      ...
      sts(i).command match {
        ...
        case CallCommand(value) =>
          val dest = resolveValue(value)
          if (lineNums.contains(dest))
            if (!runLn(lineNums(dest))) return false
            runLn(i+1)
          else
            sys.error(s"Attempt to goto non-existant line $dest")
        case EndCommand =>
          /*
          debugPrintln(s"End: $loopStack")
          loopStack.head match {
            case x@(i, WhileCommand(pred)) =>
              if (resolveValue(pred) != 0)
                if (!runLn(i)) return false
              else loopStack.pop
            case x@(i, UntilCommand(pred)) =>
              if (resolveValue(pred) == 0)
                if (!runLn(i)) return false
              else loopStack.pop
            case x@(i, ForCommand(m, RangeSpec(Number(f), Number(t), Some(Number(s)), true))) =>
              if (f.toInt until t.toInt by s.toInt contains resolveValue(m))
                assignMem(m, AdditiveOp(m, '+', Number(s)))
                if (!runLn(i)) return false
              else loopStack.pop
            case (_, f: ForCommand) => sys.error(s"Unprocessed for command $f on loop stack")
            case (_, _: IfCommand) => debugPrintln(s"Popped: ${loopStack.pop}")
          }
          */

          def findN: Int = {
            for (st <- i-1 to 0 by -1) {
              if (sts(st).command.isInstanceOf[Jump]) return st
            }
